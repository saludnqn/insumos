...
public partial class Recetas_GenerarPedido : System.Web.UI.Page
{
    ...
    protected void btnBuscar_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = SPs.InsGetListadoInternacion(fecha).GetDataSet().Tables[0];
        if (dt.Rows.Count > 0)
        {
            lblMensaje.Text = "";
            lblArea.Text = "Area: " + dt.Rows[0][1].ToString();
            lblServicio.Text = "Servicio: " + dt.Rows[0][3].ToString();
            lbImprimir.Visible = true;
        }
        else
        {
            lblMensaje.Text = "No existen Pedidos de Internación en la fecha seleccionda";
            lblArea.Text = "";
            lblServicio.Text = "";
        ...
        gvPedidos.DataBind();
        lblUsuario.Text = "Preparado por: " + fullname;
    }
    ...
    protected void lbImprimir_Click(object sender, EventArgs e)
    {
        ...
        string informe = "ListadoInternacion.rpt";
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=ListadoInternacion.pdf");
    ...
    private DataSet CargarDatos(DateTime fecha)
    {
        ...
        DataTable dtReceta = new DataTable("DSListadoInternacion");

        DataSet x = SPs.InsGetListadoInternacion(fecha).GetDataSet();
        dtReceta = x.Tables[0].Copy();
        dtReceta.TableName = "DSListadoInternacion";
