<%@ Control Language="C#" AutoEventWireup="true" CodeFile="InsumoxEfector.ascx.cs" Inherits="UserControls_InsumoxEfector" %>

<script type="text/javascript">
    $(function() {
        var idefec = $('#<%= hfIdEfector.ClientID %>').val();        
        $('#<%= txtAutoI.ClientID %>').autocomplete({
            source: '<%= ResolveUrl("~/Services/InsumoxEfector.ashx") %>' + '?idef=' + idefec,
            minlenght: 3,
